...
class DatasetWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        if item.impath and item.impath != 'in_memory_image':
            # Load the image from a file path
            img0 = read_image(item.impath)
        else:
            # Load the image from memory (assume `item` has an `image` attribute)
            img0 = item.image
        output = {
            'label': item.label,
            'domain': item.domain,
...
def build_data_loader(
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        num_workers=0,  # 设置为 0 避免多进程错误，如果仍有问题可以尝试 2 或 4
        shuffle=shuffle,
        drop_last=False,
