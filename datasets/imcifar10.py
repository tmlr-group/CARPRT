...
class IMBALANCECIFAR10(DatasetBase):
    """IMBALANCE CIFAR-10 dataset class as a subclass of DatasetBase."""

    dataset_dir = 'cifar-10-imbalance'

    def __init__(self, root, imbalance_ratio=100):
        self.dataset_dir = os.path.join(root, self.dataset_dir)
        self.imbalance_ratio = imbalance_ratio

        # Load CIFAR-10 dataset
        train_data = CIFAR10(root=self.dataset_dir, train=True, download=True)
        test_data = CIFAR10(root=self.dataset_dir, train=False, download=True)
    ...
    def _apply_imbalance(self, data):
        ...
        # 为每个类别选择保留的样本索引
        indices_to_keep = []
        class_sample_counts = {i: 0 for i in range(10)}
